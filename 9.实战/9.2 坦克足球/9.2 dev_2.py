...
def get_position_inside(src_sprite, keep_distance):
...
def get_position(src_sprite, target_pos, keep_distance):
    if target_pos == None:
        pos = get_position_inside(src_sprite, keep_distance)
...
def check_response_to_stuck(vs, hs, me, tankname):
    global run_direct
    if get_is_stuck(me, 0.3, tankname) :
...
def tank1_update():
    ...
    print(f"我1的位置 x={me.x}, y={me.y}")
    print(f"我1的速度 x={me.vx}, y={me.vy} r={opt.r2a(me.vr)}")
    print(f"我1的卡住 = {get_is_stuck(me, 0.3, tankname)}")
    print(f"我1的卡住 = {me.is_stuck(0.3)}")
    print(f"球的位置 x={ball.x}, y={ball.y}")
    print(f"球的角度 = {ball.angle}")
    print(f"球的速度 x={ball.vx}, y={ball.vy}")
...
def tank2_update():
    ...
    print(f"我2的位置 x={me.x}, y={me.y}")
    print(f"我2的速度 x={me.vx}, y={me.vy} r={opt.r2a(me.vr)}")
    print(f"我2的卡住 = {get_is_stuck(me, 0.3, tankname)}")
    print(f"我2的卡住 = {me.is_stuck(0.3)}")
    print(f"球的位置 x={ball.x}, y={ball.y}")
    print(f"球的角度 = {ball.angle}")
    print(f"球的速度 x={ball.vx}, y={ball.vy}")
    vs, hs = attack(me, target, tankname)
